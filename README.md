...
[26일차]
@Deprecated : 다음 버전에서 삭제 될수 있으니 가급적 쓰지 말아달라
protected : 동일한 패키지 내에 파생클래스만 접근 가능
private : 자기 자신의 클래스만 접근 가능

스태틱 메서드 : 인스턴스 변수를 사용안함 
객체를 생성하지 않고 호출이 가능하다

인스턴스 메서드 스태틱으로 선언되지않고 new 연산자로 시작하는거 
객체지향에서 사용됨
