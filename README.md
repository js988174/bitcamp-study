...
html-exam/ 
 ...
 톰캣 10버전 아래로 설치 > 환경 변수 설정 CATALINA_HOME 새로 만들기   >
  PATH에는 bin까지 설정 
  Server -> RunTime 환경에서 톰캣 버전 설정해주기 -> 아래 콘솔창 Servers에다 추가하고 add 해주기 
[10일차]
...
[71일차]
selectList : 목록 리턴  selectOne : 한 개의 결과 리턴 typeAliases : XML 설정에만 쓸 수 있는 별명 같은거 

[77일차]
클래스 로딩 : 스태틱 변수 준비, 스태틱 블록 실행
프록시 : 다른 무언가를 연결해주는 인터페이스의 역활

[79일차]
애노테이션 : 컴파일이나 실행할 떄 추출 할수 있고 값의 형태를 줄 수 있다.


rest 방식 : UML 을 서버에 요청하면  XML파일로 주는거

Servlet 이란 웹에서 자바 프로그래밍 구현을 위해 만들어진거, 서버 역활을 확장해주는 것 

Servlet Container : HTTP를 요청 받아서 Servlet을 실행 시키고 결과를 사용자 브라우저에게 전달해주는 것

Web Server : 클라이언트의 요청을 받아 컨테이너로 전송하고 웹 컨테이너의 결과값을 받아 
                 클라이언트로 전송 

Web Application Server : 다양한 기능을 컨테이너에 구현하고 다양한 역활을 수행하는 서버
