...
public class Exam0420 {
    ...
    public void run() {
      try {
        ...
        for (long i =0; i < 100000000; i++) {
          double r = Math.sin(1.12) * Math.cos(3.14) / Math.random();
        }
  ...
  public static void main(String[] args) {
    ...
    //    바로 스레드를 멈출 기회라고 보고 스레드 실행을 시도할 것이다.
    //    이런 이유로 shutdown() 보다는 스레드 종료를 좀 더 강제하는 효과가 있다.
    // => 작업 중인 스레득 Not Runnable 상태가 아니라면 작업이 끝날 때까지 기다린다.
    // => 이런 이유로 실행 중인 작업이 취소되는 것을 보장하지는 않는다.
